...
async function detectChallenge(browser, page, browserProxy) {
    ...
    if (
        ...
        content.includes("Checking your browser before accessing") ||
        url.includes("challenges.cloudflare.com") ||
        content.includes("cf-challenge") ||
        content.includes("managed-challenge") ||
        content.includes("turnstile")
    ) {
        colored(colors.COLOR_PINK, "Challenge detected! Bypassing proxy: " + browserProxy);

        try {
            await sleep(10); // chờ Cloudflare xử lý JS challenge
