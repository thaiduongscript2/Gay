...
const headers = {};
headers["referer"] =Math.random() < 0.77 ? `https://${parsed.host}` : refers[Math.floor(Math.random() * refers.length)];
headers["origin"] = Math.random() < 0.77 ? `https://${parsed.host}` : refers[Math.floor(Math.random() * refers.length)];
...
const architectures = {
  ...
  "Macintosh": `Intel Mac OS X 10_1${randstra(1)}_${randstra(1)}` ,
  "X11": `Linux x86_64`
};
...
const test = generateRandomString(2,5);
	 function shuffleObject(obj) {
					const keys = Object.keys(obj);
